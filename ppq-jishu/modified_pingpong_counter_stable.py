...
class RecorderThread(QThread):
    ...
    def run(self):
        ...
        self.output_file = os.path.abspath(f"output_{timestamp}{RECORD_FILE_EXT}")
...
class VideoProcessor(QThread):
    frame_processed = pyqtSignal(np.ndarray, float, int, int, int)  # frame, fps, count, processed_frames, total_frames
    processing_finished = pyqtSignal(int)
    processing_error = pyqtSignal(str)

    def __init__(self, video_path, count_line, source_width=None, source_height=None, source_fps=None):
        super().__init__()
    ...
    def _ensure_video_info(self, cap):
    ...
    def run(self):
        ...
        try:
            ...
            if not self._ensure_video_info(cap):
                raise RuntimeError("无法获取视频信息")

            self.total_frames = max(0, safe_int(cap.get(cv2.CAP_PROP_FRAME_COUNT), 0))
            ...
            while self.is_processing:
                ...
                cv2.putText(draw_frame, f"Total Frames: {self.total_frames}", (20, 158),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                cv2.putText(draw_frame, "Mode: ANALYZING", (20, 182),
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("乒乓球录制 + 自动分析计数（稳定版）")
        ...
        self.status_label = QLabel("点击“开始录制”后先保存视频；停止录制后会自动开始分析，避免边写边读 AVI 导致解析失败")
    ...
    def start_recording(self):
        ...
        self.status_label.setText("正在录制，停止后会自动开始分析...")
        self.recorder.start()

    def stop_recording(self):
        if self.recorder and self.recorder.isRunning():
            self.status_label.setText("已发出停止录制指令，等待文件写入完成后自动分析...")
            self.recorder.stop()
    ...
    def on_recording_finished(self, path, total_frames):
        ...
        self.mode_label.setText("状态: 录制结束")
        self.status_label.setText(f"录制完成，共 {total_frames} 帧；正在自动开始分析...")
        self.start_processing_loaded_video(auto_trigger=True)
    ...
    def update_recording_frame(self, frame, fps, recorded_frames):
        ...
        self.progress_label.setText(f"已录制: {recorded_frames}")
    ...
    def start_processing_loaded_video(self, auto_trigger=False):
        if not self.video_path:
            self.status_label.setText("没有可分析的视频")
            return

        if self.is_recording_now():
            self.status_label.setText("当前正在录制，请先停止录制")
            return
        ...
        if auto_trigger:
            self.status_label.setText("录制已结束，正在自动分析视频...")
        else:
    ...
    def update_processed_frame(self, frame, fps, count, processed_frames, total_frames):
        self.current_frame = frame
        ...
        self.progress_label.setText(f"处理: {processed_frames} / 总帧: {total_frames}")
        self.mode_label.setText("状态: 分析中")

    def processing_finished(self, total_count):
        self.count_label.setText(f"总计数: {total_count}")
        self.progress_label.setText(f"处理完成: {self.current_processed_frames} / {self.current_total_frames}")
